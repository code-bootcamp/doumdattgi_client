...
import { minAmountState } from "../stores";
...
export const schemaSeekCreate = yup.object({
  ...
  product_sub_category: yup.string().required("상세 카테고리를 선택해주세요."),
  product_main_text: yup.string().required("게시글 내용을 작성해주세요."),
  product_possibleAmount: yup
    .string()
    .required("신청 가능 금액을 작성해주세요."),
...
export const useSchema = () => {
  const [minAmount] = useRecoilState(minAmountState);

  const schemaCreateRequest = yup.object({
    ...
    request_price: yup
      ...
      .nullable()
      .required("작업 요청 금액을 입력해주세요.")
      .min(minAmount, `최소 신청 금액 ${minAmount}원 이상 신청할 수 있습니다.`),
